...
const localLogin =
  new LocalStrategy(localOptions,function(email, password, done) {
    ...
    User.findOne({ email: email }, function(err, user) {
      ...
      // Compare passwords - check if 'password' is equal to user.password
...
const jwtLogin = new JwtStrategy(jwtOptions, function(payload, done) {
 ...
 User.findById(payload.sub, function(err, user) {
  ...
  if (user) {  // If yes, call 'done' with that
   done(null, user);
  } else {
   done(null, false);  // Otherwise, call 'done' without an user object
  }
 });
});
